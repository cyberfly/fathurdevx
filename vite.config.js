...
function generatePortfolioItemContent(item, allItems) {
  ...
  const sourceUrl = (frontmatter.sourceUrl || "").trim();
  const hasSourceUrl = sourceUrl && sourceUrl !== "#";
  const sourceCodeButtonHtml = hasSourceUrl
    ? `
            <a
              href="${sourceUrl}"
              target="_blank"
              rel="noopener noreferrer"
              class="inline-flex items-center justify-center gap-2 px-6 py-3 bg-white text-foreground font-bold rounded-full border-2 border-border-dark hover:bg-muted transition-colors"
            >
              Source Code
              <svg
                xmlns="http://www.w3.org/2000/svg"
  ...
  return portfolioItemTemplate
    ...
    .replace(/<%-\s*sourceCodeButtonHtml\s*%>/g, sourceCodeButtonHtml)
